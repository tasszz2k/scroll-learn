// Pluck mode: shared "hold Option (Alt) and hover" capture flow used by both
// the content script (regular sites, gated by the host allowlist) and the
// dashboard / sidebar (extension's own pages, where Chrome refuses to inject
// content scripts at all).
//
...
// just gets pluck behaviour without reimplementing the watch/sync plumbing.

import type { Settings, DictionarySense, DerivedForm } from './types';
...
// DOM ids the pluck flow attaches to the document. Centralized here so the
...
export interface PluckHandle {
  ...
  // Idempotent and safe to call before mount has completed.
  unmount: () => void;
}
...
interface PluckState {
  ...
  // Pluck mode: while Option (Alt) is held, hovering shows a green outline as
  // a preview; nothing is saved. Releasing the modifier captures whichever
  ...
  // words are saved instead of the surrounding element's full text. Pressing
  // Esc cancels without saving.
  active: boolean;
  hoverEl: HTMLElement | null;
  hoverPrev: { outline: string; outlineOffset: string; backgroundColor: string } | null;
...
export function mountPluckMode(): PluckHandle {
  ...
  function toastVisibleMs(): number {
    const seconds = state.settings.noteToastDurationSeconds;
    if (!Number.isFinite(seconds) || seconds <= 0) {
      return DEFAULT_SETTINGS.noteToastDurationSeconds * 1000;
    }
  ...
  function isInsideEditable(node: Node | null): boolean {
    ...
    while (el) {
      ...
      if (
        ...
        el.className.includes('scrolllearn-')
      ) {
        return true;
      }
      el = el.parentElement;
  ...
  async function copyTextToClipboard(text: string): Promise<boolean> {
    // Try the modern clipboard API first; fall back to execCommand for sites
    // where the API is blocked by permissions-policy or the document is not
    // focused (common on preview/iframe surfaces). hasFocus() avoids a
    // guaranteed DOMException on the most frequent failure mode.
    ...
    try {
      ...
      console.warn('[ScrollLearn:pluck] clipboard fallback threw:', describeError(err));
      return false;
    }
    if (modernErr) {
      console.warn('[ScrollLearn:pluck] clipboard write blocked:', describeError(modernErr));
    }
    return false;
  }
  ...
  async function trySendNote(text: string) {
    // Stage the captured text on the system clipboard alongside the save so
    // the user can paste it elsewhere without an explicit Ctrl+C step.
    ...
    try {
      chrome.runtime.sendMessage({ type: 'save_note', note }, response => {
        if (chrome.runtime.lastError) {
          ...
          console.warn('[ScrollLearn:pluck] sendMessage failed:', msg);
          showStatusToast(`Save failed: ${msg}`, 'error');
          return;
        }
        if (!response) {
          console.warn('[ScrollLearn:pluck] sendMessage: no response');
          showStatusToast('Save failed: no response', 'error');
          return;
        }
        if (!response.ok) {
          console.warn('[ScrollLearn:pluck] save_note error:', response.error);
        ...
        console.log(
          '[ScrollLearn:pluck] save_note ok, clipboard:', copied,
          'translation:', !!translation,
          'senses:', senses?.length ?? 0,
          'family:', derivedForms?.length ?? 0,
        );
        showToast(text, { copied, translation, senses, derivedForms });
      });
    } catch (err) {
      console.warn('[ScrollLearn:pluck] sendMessage threw:', err);
  ...
  function setPluckHover(el: HTMLElement | null) {
    ...
    if (state.hoverEl && state.hoverPrev) {
      state.hoverEl.style.outline = state.hoverPrev.outline;
      state.hoverEl.style.outlineOffset = state.hoverPrev.outlineOffset;
      state.hoverEl.style.backgroundColor = state.hoverPrev.backgroundColor;
    }
    state.hoverEl = el;
    state.hoverPrev = null;
    if (el) {
      state.hoverPrev = {
  ...
  function pluckExtractText(target: HTMLElement): string {
    // Prefer the click target itself, but if it has no text walk up to find
    // one. For buttons/links wrap inner spans, climbing to the button gives
    // the full label.
  ...
  function onPluckMove(e: MouseEvent) {
    ...
    // While the user is dragging to select text, suppress the hover outline
    // so it doesn't compete visually with the native selection highlight.
    if (state.mouseDown) {
      setPluckHover(null);
      return;
    }
  ...
  function pluckCaptureElement(target: HTMLElement) {
    if (target.closest(`#${TOAST_STACK_ID}`)) return;
    const text = pluckExtractText(target).replace(/\s+/g, ' ').trim();
    if (!text) {
      console.log('[ScrollLearn:pluck] no text extracted from', target);
      return;
    }
    if (text.length < state.settings.noteMinLength) {
      console.log('[ScrollLearn:pluck] text below minLength', text);
      return;
    }

    const now = Date.now();
    if (text === state.lastSentText && now - state.lastSentAt < LOCAL_DEDUPE_MS) {
      console.log('[ScrollLearn:pluck] dedupe skip', text);
      return;
    }
    state.lastSentText = text;
    state.lastSentAt = now;

    void trySendNote(text);
  }
  ...
  function pluckCaptureSelection(): boolean {
    ...
    if (isInsideEditable(sel.anchorNode)) {
      console.log('[ScrollLearn:pluck] selection inside editable, skip');
      return false;
    }
    const text = raw.replace(/\s+/g, ' ').trim();
    if (text.length < state.settings.noteMinLength) {
      console.log('[ScrollLearn:pluck] selection below minLength', text);
      return false;
    }

    const now = Date.now();
    if (text === state.lastSentText && now - state.lastSentAt < LOCAL_DEDUPE_MS) {
      console.log('[ScrollLearn:pluck] selection dedupe skip', text);
      return false;
    }
    state.lastSentText = text;
    state.lastSentAt = now;
  ...
  function pluckCaptureSelectionOrElement(target: HTMLElement | null): boolean {
    ...
    if (target.closest(`#${TOAST_STACK_ID}`)) return false;
  ...
  function onPluckMouseUpCapture() {
    ...
    if (state.sessionCaptured) return;

    // If the user drag-selected text during the modifier hold, save just
    // those words. A bare click (no selection) is handled by the click
    // listener below.
  ...
  function onPluckClickCapture(e: MouseEvent) {
    ...
    // mouseup may have already captured a drag-selected range during this
    // hold. Consume the click so the page's link/button handler doesn't fire
    // either.
    ...
    const target = (e.target as HTMLElement | null) ?? state.hoverEl;
    if (!target) return;
    if (target.closest(`#${TOAST_STACK_ID}`)) return;
  ...
  function startPluck() {
    ...
    console.log('[ScrollLearn:pluck] mode ON');
  }

  function stopPluck(opts: { skipCapture?: boolean } = {}) {
    if (!state.active) return;
    // On release, prefer any drag-selected text over the hovered element.
    // Skip entirely if click/contextmenu/mouseup already captured this
    // session or the caller asked us to (e.g. window blur).
    const shouldCapture = !opts.skipCapture && !state.sessionCaptured;
    const captureTarget = shouldCapture ? state.hoverEl : null;
  ...
  function onPluckKeyDown(e: KeyboardEvent) {
    ...
    if (e.key === 'Escape' && state.active) {
      console.log('[ScrollLearn:pluck] cancelled via Escape');
      stopPluck({ skipCapture: true });
      return;
    }
    if (!isPluckModifier(e)) return;
    if (isInsideEditable(document.activeElement)) {
      console.log('[ScrollLearn:pluck] skipped: focus is in an editable field');
      return;
    }
    startPluck();
  }
  ...
  // Settings hydration: initial load + live-watch chrome.storage so changes
  // propagate without a reload. Both are best-effort -- if chrome.storage is
  // unavailable (e.g. test runner) we silently keep DEFAULT_SETTINGS.
  async function loadSettings() {
    try {
      const stored = await chrome.storage.local.get(STORAGE_KEYS.SETTINGS);
      const partial = (stored[STORAGE_KEYS.SETTINGS] as Partial<Settings> | undefined) ?? {};
      state.settings = { ...DEFAULT_SETTINGS, ...partial };
    } catch {
      // ignore
    }
  }
  ...
  // Wire everything up. The storage hydration is async but the listeners are
  ...
  return {
    unmount() {
      ...
      console.log('[ScrollLearn:pluck] unmounted');
    },
  };
}
